...
class saifa_Breadcrumbs {
	...
	public function __construct( $templates = array(), $options = array(), $strings = array(), $autorun = true ) {
		...
		$this->strings = wp_parse_args( $strings, array(
			'home' => __( 'Home', 'saifa' ),
			'search' => array(
				'singular' => __( 'Search Result for <em>%s</em>', 'saifa' ),
				'plural'   => __( '%s Search results for <em>%s</em>', 'saifa' ),
			),
			'paged' => __( 'page %d', 'saifa' ),
			'404_error' => __( 'Error: Page not found', 'saifa' ),
		) );
